 ...
 * 		-> 이전의 가장 작은 값을 유지, 더 작은 가격이 나올 때까지 도로의 길이를 더해줌
 * 	결국, 이전 가격과 다음 가격을 비교하는 과정이 필요
 */
public class boj13305_주유소_MySol {
	...
	public static void main(String[] args) throws NumberFormatException, IOException {
		...
		for (int i = 0; i < priceArr.length-1; i++) {
			...
			if (minCost <= priceArr[i]) {
				System.out.println("minCost(" + minCost + ") <= 현재 도로의 가격(" + priceArr[i] + ")");
			...
			// "현재 도로의 가격 > minCost"일 경우 -> 현재 가격으로 minCost를 변경, 더한 도로의 길이 초기화
				...
				System.out.println("현재 도로의 가격(" + priceArr[i] + ") > minCost(" + minCost + ")");
				tempSum = sumLoad * minCost;
